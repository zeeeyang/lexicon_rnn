...
void ReadExample(const std::string& line, Example& example, bool debug = false)
{
    ...
    while(in) {
        ...
        if(is_word)
        {
            ...
            int labelId = tag_dict.Convert(word);
            assert(labelId != -1);
            example.label = labelId;
...
int main(int argc, char** argv) {
    ...
    cnn::Initialize(argc, argv);
    if ( argc != 6) {
        cerr << "Usage: " << argv[0] << " vec lex train dev test\n";
        return 1;
    }
    vector<Example> training_examples, dev_examples, test_examples;
    string line;
    ...
    cerr << "Labels: " << tag_dict.size() << endl;


    int dlc = 0;
    int dtoks = 0;
    ReadExamples(argv[4], dev_examples, dlc, dtoks);

    int telc = 0;
    int tetoks = 0;
    ReadExamples(argv[5], test_examples, telc, tetoks);
