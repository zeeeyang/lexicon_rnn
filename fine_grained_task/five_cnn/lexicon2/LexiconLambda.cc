...
struct LSTMClassifier {
    ...
    Expression BuildGraph(Example& example, ComputationGraph& cg, float& cor) {
        ...
        if(eval)
        {
            auto example_sentiment_score = as_scalar(cg.incremental_forward());
            if(verbose)
                cerr<<"averaged score:\t"<< example_sentiment_score<<endl;
            if(verbose)
                cerr<<"gold label: " << example.label<<endl;
            if( example_sentiment_score < -1.5 && example.label == -2  )
                cor++;
            else if (example_sentiment_score >=-1.5 && example_sentiment_score < -0.5 && example.label == -1)
                cor++;
            else if (example_sentiment_score >=-0.5 && example_sentiment_score < 0.5 && example.label == 0)
                cor++;
            else if (example_sentiment_score >=0.5 && example_sentiment_score < 1.5 && example.label == 1)
                cor++;
            else if (example_sentiment_score >=1.5 && example.label == 2)
                cor++;
            else if(verbose) cerr<<"\t[ERROR]" ;
            if(verbose)
                cerr<< endl;
        }
        Expression goldy = input(cg, &example.label);
...
void evaluate(vector<Example>& examples, LSTMClassifier& lstmClassifier, float& acc)
{
    ...
    for (auto& sent : examples) {
        ...
        ComputationGraph cg;
        Expression y_pred = lstmClassifier.BuildGraph(sent, cg, num_correct);
...
int main(int argc, char** argv) {
    ...
    for(unsigned sent_index = 0; sent_index < training_examples.size(); sent_index++)
    {
        auto& example = training_examples[ sent_index ];
        example.isSingletons.resize(example.words.size(), false);
        for(unsigned word_index = 0; word_index < example.words.size(); word_index++)
        {
            if(singletons.find(example.real_word_ids[word_index]) != singletons.end())
            {
               example.isSingletons[word_index] = true; 
            }    
        }
    }
    ...
    while(count<=30) {
        ...
        for (unsigned i = 0; i < report_every_i; ++i) {
            ...
            {
                if(sentx_y.isSingletons[j] && cnn::rand01() < punk)
                    sentx_y.word_ids[j] = kUNK;
                else
                    sentx_y.word_ids[j] = sentx_y.real_word_ids[j];
            }
            const int y = sentx_y.label;
            Expression y_pred = lstmClassifier.BuildGraph(sentx_y, cg, tcorr);
            loss += as_scalar(cg.incremental_forward());
            cg.backward();
            sgd->update(1.0);
            ++si;
