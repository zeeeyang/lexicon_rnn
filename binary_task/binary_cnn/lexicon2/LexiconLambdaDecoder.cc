...
struct LSTMClassifier {
    ...
    Expression BuildGraph(Example& example, ComputationGraph& cg, float& doc_score) {
...
void ReadExample(const std::string& line, Example& example, bool needOffset=false, bool debug = false)
{
    ...
    while(in) {
        ...
        if(is_word)
        {
            ...
            if(needOffset)
            {
                //for decoder, the input label  1 is for positive, 0 is for negative
                if(example.label == 1)
                    example.label = 3;
                else{
...
void ReadExamples(const string& inputFile, vector<Example>& examples, int& lines, int& tokens, bool needOffset=false)
{
    string line;
    cerr << "Reading data from " << inputFile << "...\n";
    {
        ...
        while(getline(in, line)) {
            Example example;
            ReadExample(line, example, needOffset);
            if(example.label == 0)
                continue;
            if(example.label >0)
                example.label = 1;
...
void evaluate(vector<Example>& examples, LSTMClassifier& lstmClassifier, int& num_neg, int& num_pos, float& doc_score)
{
    eval = true;
    float num_correct = 0;
    float loss = 0;
    int label = examples[0].label; //document label
    for (auto& sent : examples) {
        const int y = sent.label;
        assert(y == label);
        ComputationGraph cg;
        Expression y_pred = lstmClassifier.BuildGraph(sent, cg, doc_score);
    ...
    {
        num_neg = num_correct;
        num_pos = examples.size() - num_neg;
    }
    else {
        num_pos = num_correct;
        num_neg = examples.size() - num_pos;
    }
    eval = false;
}
...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i< argc; i++)
        cerr<< i<<":\t" << argv[i] << endl; 
    if ( argc != 10) {
        cerr << "Usage: " << argv[0] << " vec lex train.phrase train dev test model predict output\n";
        return 1;
    }
    ...
    string model_name = argv[7];
    string predict_file = argv[8];
    string output_file = argv[9];
    Model model;
    ...
    cerr<< "Load Model ..." << endl;
    ifstream in(model_name);
    boost::archive::text_iarchive ia(in);
    ia >> model;

    cerr<< "Load Predict File ..." << predict_file << endl;
    vector<Example> predicted_examples;
    int plc = 0;
    int ptoks = 0;
    ReadExamples(predict_file, predicted_examples, plc, ptoks, true);


    int num_predict_unk = 0;
    //HANDLE UNK in the dev corpus: tzy
    for(unsigned sent_index = 0; sent_index < predicted_examples.size(); sent_index++)
    {
        auto& example = predicted_examples[ sent_index ];
        for(unsigned word_index = 0; word_index < example.words.size(); word_index++)
        {
            if(example.word_ids[word_index] == kUNK)
                num_predict_unk ++;
    ...
    cerr<<"NUM PREDICTED UNK: " << num_predict_unk << endl;

    ofstream fout(output_file);
    int num_neg = 0, num_pos = 0;
    float doc_score = 0;
    evaluate(predicted_examples, lstmClassifier, num_neg, num_pos, doc_score);
    fout << predicted_examples[0].label << "\t" << num_neg << "\t" << num_pos << "\t" << doc_score << endl;
    fout.close();
    delete sgd;
}
